...
import { getDoctorDiaryReports } from '../../utils/api';

export default function DoctorDiaryReports() {
    ...
    useEffect(() => {
        async function fetchReports() {
            setLoading(true);
            try {
                const res = await getDoctorDiaryReports();
                if (res.success) {
    ...
    if (loading) {
        return (
            <DashboardLayout role="doctor" title="Diary Reports">
    ...
    return (
        <DashboardLayout role="doctor" title="Diary Reports">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '1px solid var(--gray-200)', paddingBottom: 16, marginBottom: 28 }}>
                <div>
                    <h2 style={{ fontSize: 22, fontWeight: 700, color: 'var(--gray-800)', marginBottom: 4 }}>Voice Diary Analysis</h2>
            ...
            {reports.length === 0 ? (
                <div style={{ padding: 48, textAlign: 'center', color: 'var(--gray-400)', fontStyle: 'italic' }}>
                    No diary entries found for your assigned patients.
                </div>
            ) : (
                                ...
                                <div style={{ display: 'flex', gap: 12 }}>
                                    {patient.weekly_summary.crisis_count > 0 && (
                                        <span style={{ fontSize: 11, background: 'var(--danger-light)', color: 'var(--danger)', padding: '6px 12px', borderRadius: 8, fontWeight: 700, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6 }}>
                                            ⚠️ {patient.weekly_summary.crisis_count} Crisis Alert{patient.weekly_summary.crisis_count > 1 ? 's' : ''}
                                        </span>
                                    )}
