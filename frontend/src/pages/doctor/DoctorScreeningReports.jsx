...
function ClinicalReport({ assessment, patientName, patientEmail }) {
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        boxShadow: `0 4px 16px ${c.color}30`, flexShrink: 0,
                    }}>
                ...
                <div style={{ padding: '32px 36px' }}>
                    ...
                    <div style={{ background: '#E8F1F8', borderRadius: 12, padding: 24, marginBottom: 28, border: '1px solid #D0E4F0' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
                            <div>
                                ...
                                <p style={{ fontSize: 15, fontWeight: 600, color: '#1F2F3D' }}>{patientName}</p>
                            </div>
                            <div>
                                <p style={{ fontSize: 12, color: '#94A3B5', fontWeight: 500 }}>Email</p>
                                <p style={{ fontSize: 15, fontWeight: 600, color: '#2A6F97' }}>{patientEmail || '—'}</p>
                            </div>
                            <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        ...
                        <button className="btn btn-secondary" style={{ padding: '16px 0', borderRadius: 10, fontSize: 15 }} onClick={() => window.location.href = '/doctor/patients'}>
...
export default function DoctorScreeningReports() {
    ...
    useEffect(() => {
        apiFetch('/doctor/patients/')
            .then(r => r.json())
            .then(d => { if (d.success) setPatients(d.patients || []); })
            .catch(() => { })
            .finally(() => setLoading(false));
    }, []);

    function loadAssessments(patientId) {
        setLoadingDetail(true);
        setAssessments([]);
        setActiveAssessment(null);
        apiFetch(`/doctor/patient/${patientId}/`)
            .then(r => r.json())
            .then(d => {
    ...
    function handleSelectPatient(p) {
        setSelected(p);
        loadAssessments(p.id);
    }

    return (
        <DashboardLayout role="doctor" title="AI Screening Reports">
            <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 24, alignItems: 'start' }}>
                ...
                <div style={{
                    ...
                    <div style={{ padding: '20px 18px 14px', borderBottom: '1px solid #F0F2F5' }}>
                        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#1F2F3D', display: 'flex', alignItems: 'center', gap: 8 }}>
                            🧠 Screening Reports
                        </h3>
                        <p style={{ fontSize: 12, color: '#94A3B5', marginTop: 3 }}>Select a patient to view their AI screening results</p>
                    </div>
                    <div style={{ padding: '10px 12px', maxHeight: '70vh', overflowY: 'auto' }}>
                        {loading ? (
                            <div style={{ textAlign: 'center', padding: '40px 0', color: '#94A3B5', fontSize: 13 }}>Loading patients…</div>
                        ) : patients.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '40px 0', color: '#94A3B5', fontSize: 13 }}>No patients found.</div>
                        ) : patients.map(p => (
                            <button key={p.id} onClick={() => handleSelectPatient(p)} style={{
                                width: '100%', textAlign: 'left',
                                padding: '11px 12px', borderRadius: 10,
                                border: 'none', cursor: 'pointer', fontFamily: 'inherit',
                                background: selected?.id === p.id ? '#EBF4FB' : 'transparent',
                                borderLeft: selected?.id === p.id ? '3px solid #2A6F97' : '3px solid transparent',
                                transition: 'all 0.15s', marginBottom: 2,
                            }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <div style={{
                                        width: 36, height: 36, borderRadius: '50%', flexShrink: 0,
                                        background: selected?.id === p.id ? '#2A6F97' : '#E8F1F8',
                                        color: selected?.id === p.id ? '#fff' : '#2A6F97',
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        fontSize: 14, fontWeight: 700,
                                    }}>{(p.name || p.username || 'P')[0].toUpperCase()}</div>
                                    <div>
                                        <p style={{ fontSize: 14, fontWeight: 600, color: '#1F2F3D', margin: 0 }}>{p.name || p.username}</p>
                                        <p style={{ fontSize: 11, color: '#94A3B5', margin: 0 }}>{p.email || ''}</p>
                                    </div>
                                </div>
                            </button>
                        ))}
                ...
                <div>
                    {!selected ? (
                        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E2E7ED', padding: '80px 40px', textAlign: 'center' }}>
                            ...
                            <h3 style={{ fontSize: 20, fontWeight: 700, color: '#1F2F3D', marginBottom: 8 }}>Select a Patient</h3>
                            <p style={{ fontSize: 15, color: '#6B7D8F' }}>Choose a patient from the left panel to view their AI screening report.</p>
                        </div>
                    ) : loadingDetail ? (
                        ...
                        <>
                            ...
                            {activeAssessment && (
                                <ClinicalReport
                                    ...
                                    patientName={selected.name || selected.username}
