...
class Patient(models.Model):
    ...
    # Bhavi v2.0: Subscription tier
    tier = models.CharField(max_length=10, choices=TIER_CHOICES, default='free')

    # Bhavi v2.0: Learned personality notes (Plus only — generated by personalization pipeline)
...
class ChatSession(models.Model):
    """Stores individual conversation sessions for a patient."""
...
class ChatHistory(models.Model):
    ...
    emotion_score = models.FloatField(null=True, blank=True)              # mood score 1-10
    created_at  = models.DateTimeField(auto_now_add=True)
...
class EmotionalSummary(models.Model):
    """
    Stores periodic emotional summaries for long-term pattern tracking.
    Generated by the Bhavi AI pipeline at regular intervals.
    """
    RISK_CHOICES = [
        ('low', 'Low Risk'),
        ('medium', 'Medium Risk'),
    ...
    patient           = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='emotional_summaries')
    summary_text      = models.TextField()                           # AI-generated emotional summary
    ...
    def __str__(self):
        return f"Emotional Summary for {self.patient.name}: {self.dominant_emotion} ({self.period_start.strftime('%Y-%m-%d')})"
...
# ─── Bhavi v2.0: Voice Quota Tracking ────────────────────────────────────────

class VoiceQuota(models.Model):
    """
    Tracks daily voice usage per patient.
    The 24-hour window starts from first_use_at (not midnight).
    Free tier: 3 min/day. Plus tier: 20 min/day.
    """
    patient            = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='voice_quotas')
    quota_date         = models.DateField()                         # calendar date of this record
    first_use_at       = models.DateTimeField(null=True, blank=True) # 24-hr window starts here
    reset_at           = models.DateTimeField(null=True, blank=True) # = first_use_at + 24hrs
    minutes_used       = models.FloatField(default=0.0)              # cumulative voice minutes today
    upgrade_msg_shown  = models.BooleanField(default=False)          # shown once per quota period
    ...
    def __str__(self):
        return f"VoiceQuota for {self.patient.name} on {self.quota_date}: {self.minutes_used:.1f} min used"


# ─── Bhavi v2.0: Parsed Memory Facts ─────────────────────────────────────────

class BhaviMemoryFact(models.Model):
    """
    Stores individual facts parsed from Bhavi's [MEMORY_UPDATE: ...] tags.
    These are important life facts the user shared that Bhavi should remember.
    e.g. "Granddaughter Kavya got first rank in class"
    """
    patient    = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='memory_facts')
    fact       = models.TextField()                    # the extracted fact text
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"MemoryFact for {self.patient.name}: {self.fact[:80]}"
